...
export class Editor {
  ...
  _move(ev) {
    ...
    // 幽灵预览
    if (['roach', 'locust', 'scarab', 'snail', 'fly', 'firecracker', 'skyrocket', 'bottle', 'brick', 'glass', 'sponge'].includes(this.tool)) {
      this.ghost = {
        t: this.tool,
        kind: BUG_TYPES.includes(this.tool)
          ? 'bug'
          : this.tool === 'brick' || this.tool === 'glass' || this.tool === 'sponge'
            ? 'prop'
            : 'explosive',
        x,
        y,
  ...
  _canPlace(t) {
    const kind = BUG_TYPES.includes(t) ? 'bug' : t === 'brick' || t === 'glass' || t === 'sponge' ? 'prop' : 'explosive';
